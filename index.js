...
async function processQueue() {
  ...
  isProcessing = true;
  const { trackingNumber, res } = queue.shift();
...
app.get("/api/track", async (req, res) => {
  ...
  if (!trackingNumber) {
    return res.status(400).json({ error: "Tracking number required", status: "bad_request" });
  }

  console.log(`🕓 Queuing request for ${trackingNumber}`);
  queue.push({ trackingNumber, res });
